...
#CPK: Oh, I see what you did. You summarized and then joined the summary table back to the original data so you could compute se. See below.
...
pseed.sum.max <- pseed.wide %>% 
  ...
  summarize(amp.sum.max = max(amp.sum))%>%
  group_by(fish,bl.s)%>%
  summarize(amp.sum.mean=mean(amp.sum.max),amp.sum.se=SE(amp.sum.max))
...
#CP: Great work, esp on these last few prompts. Keep it going.